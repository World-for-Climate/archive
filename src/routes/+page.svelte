<script>
	...
	$: topics = $question_store
		.flatMap((q) => q.topics)
		.filter((t, i, a) => t !== null && a.indexOf(t) === i)
		.sort();
	...
	$: questions = (
		...
		(q) =>
			$topics_filter_store.length === 0 || q.topics.some((t) => $topics_filter_store.includes(t))
	);
	...
	<div class="collapse collapse-arrow border">
		...
		<div class="collapse-title font-medium">
			{#if $topics_filter_store.length > 0 && !topics_open}
				{#each $topics_filter_store as t}
					<button
						class="btn btn-xs btn-outline btn-primary m-1 rounded-full btn-active"
						on:click={() => topics_filter_store.remove(t)}>{t}</button
					>
				{/each}
			{:else}
		...
		<div class="collapse-content">
			...
			{#each unselected_topics as t}
				<button
					class="btn btn-xs btn-outline btn-primary m-1 rounded-full"
					on:click={() => topics_filter_store.add(t)}>{t}</button
				>
			{/each}
		</div>
	</div>
	<button
		class="btn btn-warning ml-2 mt-2 mb-auto"
		on:click={() => {
			$query_store = '';
